...
object DwsOrderWideApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val offsetMapForKafkaOrderInfo: Map[TopicPartition, Long] =
      OffsetManager.getOffset(topicOrderInfo, groupId)
    val offsetMapForKafkaOrderDetail: Map[TopicPartition, Long] =
    ...
    if (offsetMapForKafkaOrderInfo != null &&
      offsetMapForKafkaOrderInfo.size > 0) { //根据是否能取到偏移量来决定如何加载kafka 流
    ...
    if (offsetMapForKafkaOrderDetail != null &&
      offsetMapForKafkaOrderDetail.size > 0) { //根据是否能取到偏移量来决定如何加载kafka 流
    ...
    val inputGetOffsetDstreamOrderDetail:
      ...
      recordInputDstreamOrderDetail.transform { rdd => //周期性在driver中执行
    ...
    val orderDetailDstream: DStream[OrderDetail] =
      inputGetOffsetDstreamOrderDetail.map { record =>
...
//    orderInfoWithKeyDstream.join(orderDetailWithKeyDstream).print(1000)
    ...
    val orderInfoWindowDstream: DStream[OrderInfo] =
      orderInfoDstream.window(Seconds(10),Seconds(5))
    val orderDetailWindowDstream: DStream[OrderDetail] =
      orderDetailDstream.window(Seconds(10),Seconds(5))


    val orderInfoWindowWithKeyDstream: DStream[(Long, OrderInfo)] =
      orderInfoWindowDstream.map(orderInfo=>(orderInfo.id,orderInfo))
    val orderDetailWindowWithKeyDstream: DStream[(Long, OrderDetail)] =
      orderDetailWindowDstream.map(orderDetail=>(orderDetail.order_id,orderDetail))

    val orderTupleDstream: DStream[(Long, (OrderInfo, OrderDetail))] =
      orderInfoWindowWithKeyDstream.join(orderDetailWindowWithKeyDstream)

    val orderWideDstream: DStream[OrderWide] =
      orderTupleDstream.map{
    ...
    val filteredOrderWideDstream: DStream[OrderWide] =
      ...
      if (orderWideList != null && orderWideList.size > 0) {
        ...
        for (orderWide <- orderWideList) {
          ...
          val isNonExists: Long =
            jedis.zadd(key, ts, orderWide.order_detail_id.toString)
          if (isNonExists == 1L) {
